...
- 以太网--IEEE802.3 系列标准
- 令牌环网--物理上是星形拓扑结构,逻辑上是环形拓扑结构
- FDDI 网--物理上采用双环拓扑结构,逻辑上是环形拓扑结构
- ATM 网--使用 53 字节固定长度的单元进行交换
...
- IEEE 802.5--令牌环网的介质访问控制协议(CSMA/CA)及物理层技术规范
- IEEE 802.8--光纤分布数字接口(FDDI)的介质访问控制协议(CSMA/CD)及物理层技术规范
...
广播域--某站点发出一个广播信号,所有广播域内的站点都会收到该广播信号
...
所传送的分组可能出错,丢失,重复,失序或超时但路由器简单,价格低廉

#### 1.异构网结互连

将两个以上的计算机网络,通过一定的方法,用一些中间设备(又称中继系统)相互连接起来,以构成更大的网络系统
...
使用物理层或数据链路层的中继系统时,只是把一个网络扩大了,从网络层的角度看,仍是同一个网络,一般不称为网络互连

可以把互连后的网络视为一个虚拟 IP 网络(相当于抽象,看不到实现细节)

#### 2.路由与转发

路由器主要完成两个功能--

- 路由选择(确定路径)--根据特定的路由选择协议构造出路由表,同时经常或定期地和相邻路由器交换路由信息而不断地更新和维护路由表
- 分组转发(当一个分组到达时所采取的动作)--处理通过路由器的数据流,主要转发表查询,转发及相关的队列管理和任务调应

#### 3.SDN

优点--

- 全局集中式控制和分布式高速转发,有利于控制平面的全局代化和高性能的网络转发
- 灵活可编程与性能的平衡,控制和转发功能分离后,使得网络可以由专有的自动化工具以编程方式配置
- 隆低成本,控制和数据平面分离后,使用开放的接口协议后,实现了网络设备的制造与功能软件的开发相分离,有效降低了成本

缺点--

- 安全风险--集中管理容易被攻击,如果崩溃,整个网络会受到影响
- 瓶颈问题,原本分布式的控制平面集中化后,随着网终规模扩大,控制器可能成为网络性能的瓶颈

#### 4.拥塞控制

拥塞--在通信子网中,因出现过量的分组而引起网络性能下降

轻度拥塞--随着网络负载的增加,网络的吞吐量明显小于正常的吞吐量状态
...
流量控制和拥塞控制的区别--流量控制是指在发送端和接收端之间的点对点通信量的控制,流量控制所要做的是抑制发送端发送数据的速率,以便使接收端来得及接收.拥塞控制必须确保通信子网能够传送待传送的数据,是一个全局性的问题,涉及网络中所有的主机,路由器及导致网络传输能力下降的所有因素

拥塞控制的方法--

- 开环控制--在设计网络时事先将有关发生拥塞的因素考虑周到,力求网络在工作时不产生拥塞.是静态预防方法,在做决定时不考虑当前网络的状态
- 闭环控制--事先不考虑有关发生拥塞的各种因素,采用监测网络系统去监视,然后将拥塞信息传到合适的地方,以便调整网络系统的运行.基于反馈环路的概念,是动态方法

### 2.数据交换

#### 1.分组交换

分组交换--将数据分割成若干个分组,并在网络中传送这些分组,接收端在收到分组后将这些分组重新组合起来,形成原来的数据
...
电路交换--在通信子网中,在通信时,双方在相邻的结点之间建立一条专用的电路,在该电路上传送数据,当数据传输结束后,双方在电路中释放该电路
...
- 静态路由算法(非自适应路由算法)--路由器在安装完毕后,就将手工配置网络的路由信息写入路由表中.当网络的拓扑结构或链路的状态发生变化时,需要手工修改路由信息
- 动态路由(自适应路由算法)--路由表项通过相互连接的路由器之间彼此交换信息,按照一定的算法优化得出.会在一定时间间隙里不断更新以适应不断变化的网络

静态路由算法简便和开销较小
动态路由算法改善网络的性能,有助于流量控制但会增加网络的负担
...
所有的结点都监听从其他结点传来的路由选择更新信息,并在以下情况更新路由选择表--

- 被通告一条新的路由,该路由在本结点的路由表中不存在,此时本地系统加入这条新的该路由
- 发来的路由信息中有一条到达基个目的地的路由,与当前使用的路由相比,有较短的距离(较小的代价).用经过发送路由信息的结点的新路由替换路由

距离-向量路由算法的实质--迭代计算一条路由中的站段数或延迟时间,从而得到到达一个目标的最短(最小代价)通路.要求每个结点在每次更新时都将它的全部路由表发送给所有相邻的结点.更新报文的大小与通信子网的结点个数成正比,大的通信子网将导致很大的更新报文.由于更新报文发给直接邻接的结点,所有结点都将参加路由选择信息交换,在通信子网上传送的路由选择信息的数量很容易变得非常大
